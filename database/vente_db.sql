...
DELIMITER //
CREATE PROCEDURE `Gestion_depot` (IN `MatricProduits` INT, IN `quantite` INT, IN `type_operation` ENUM('ENTREE','SORTIE','RETOUR'), IN `PAu` FLOAT)
   BEGIN
    ...
    IF type_operation = 'ENTREE' THEN  
        SELECT `produit_mat` into produit_id FROM `depot` WHERE produit_mat=MatricProduits
        LIMIT 1;
    ELSEIF type_operation = 'SORTIE' THEN
    	SELECT `produit_mat` into produit_id FROM `depot` WHERE produit_mat=MatricProduits
        LIMIT 1;
    ELSEIF type_operation='RETOUR' THEN
     SELECT `produit_mat` INTO produit_id FROM `depot` WHERE idproduit=tmpid
     LIMIT 1;
    END IF;
    -- stock actuel
    ...
    WHERE produit = produit_id;    
     -- prix actuel
    SELECT PAm into prix_actuel from depot where produit = produit_id;  
    -- TESTER LES VALEURS
    ...
    -- prix total actuel  
    SELECT (PAm* Stock_dispo) into Prix_total  FROM `depot` WHERE produi=produit_id;
    -- ISNULL
    ...
    IF type_operation = 'ENTREE' THEN
        ...
        set prix_tmp=PAu;
        UPDATE depot	SET Stock_dispo = stock_actuel, PAm=CUMP	WHERE produit = produit_id;    
    ELSEIF type_operation = 'SORTIE' THEN
        SET stock_actuel = stock_actuel - quantite;
...
CREATE PROCEDURE `gestion_magasin` (IN `MatricProduits` INT, IN `quantite` INT, IN `type_operation` ENUM('ENTREE','SORTIE','RETOUR'), IN `PAu` FLOAT)
   BEGIN
    ...
    -- IDPRODUITS
    IF type_operation = 'ENTREE' THEN  
        SELECT `produit_mat` into produit_id FROM `magasin` WHERE produit_mat=MatricProduits
        LIMIT 1;
    ELSEIF type_operation = 'SORTIE' THEN
    	SELECT `produit_mat` into produit_id FROM `magasin` WHERE produit_mat=MatricProduits
        LIMIT 1;
    ELSEIF type_operation='RETOUR' THEN
     SELECT `produit_mat` INTO produit_id FROM `magasin` WHERE idproduit=tmpid
     LIMIT 1;
    END IF;
    -- stock actuel
    ...
    WHERE produit = produit_id;    
     -- prix actuel
    SELECT PAm into prix_actuel from magasin where produit = produit_id;  
    -- TESTER LES VALEURS
    if prix_actuel is null THEN
    	set prix_actuel=0;
    end if;
    ...
    -- prix total actuel  
    SELECT (PAm* Stock_dispo) into Prix_total  FROM `magasin` WHERE produi=produit_id;
    -- ISNULL
    ...
    IF type_operation = 'ENTREE' THEN
    	...
    	set CUMP =SFPrix/SFentree;
     	SET stock_actuel = stock_actuel + quantite;         
       	set prix_actuel=CUMP;
        set prix_tmp=PAu;
        UPDATE magasin	SET Stock_dispo = stock_actuel, PAm=CUMP	WHERE produit = produit_id;    
    ELSEIF type_operation = 'SORTIE' THEN
        SET stock_actuel = stock_actuel - quantite;
        set prix_tmp=prix_actuel;        
...
-- GESTION DES CLIENTS
DELIMITER //
CREATE PROCEDURE gestion_clients(IN type ENUM('INSERT','UPDATE','DELETE'),in idClient varchar(14),IN noms varchar(255),in prenoms varchar(255),in phones varchar(20),in mails varchar(20),in adresses varchar(20))
BEGIN
...
if type = 'INSERT' THEN
    ...
    WHERE LEFT(MatriClient, 10) = CONCAT(prefix, year, month);
    
    IF counter > 0 THEN
        -- Récupérer le dernier numéro utilisé dans la clé primaire
        SELECT MAX(RIGHT(MatriClient, LENGTH(id_patient) - 10)) INTO counter
        FROM clients
        WHERE LEFT(MatriClient, 10) = CONCAT(prefix, year, month);
...
INSERT INTO clients (MatriClient,Nom,prenom,telephone,email,adresse) VALUES (
noms, prenoms, phones, mails, adresses
);
ELSEIF type = 'UPDATE' THEN
UPDATE clients SET Nom=noms,prenom=prenoms,telephone=phones,email=mails,adresse=adresses
WHERE MatriClient=idClient;

ELSEIF type = 'DELETE' THEN
DELETE FROM produits WHERE `produits`.`Matricule` = idClient;
END IF;
END //
DELIMITER //
...
if type = 'INSERT' THEN
    SET prefix = 'DE'; -- Préfixe de la clé primaire
    SET year = YEAR(CURDATE());
    SET month = LPAD(MONTH(CURDATE()), 2, '0'); -- Retrieve and pad current month
    SET counter = 1;
...
DELIMITER //
CREATE PROCEDURE gestion_det_achat(IN type ENUM('INSERT','UPDATE','DELETE'),in codedetail varchar(14),in produits_code varchar(14),in PA_unitaire decimal(10,4),in PVu decimal(10,4),in Quantites int,in Facturevente varchar(20))
BEGIN
DECLARE prefix VARCHAR(8); -- Modified prefix length to accommodate month
    DECLARE year VARCHAR(4);
    DECLARE month VARCHAR(2);
...
DELIMITER //
CREATE PROCEDURE Gestion_fourni(IN type ENUM('INSERT','UPDATE','DELETE'),in codefourni varchar(14),in nom varchar(50),in Prenom varchar(20),in email varchar(20),in teelphone varchar(20),in adresse varchar(20))
BEGIN
DECLARE prefix VARCHAR(8); -- Modified prefix length to accommodate month
    DECLARE year VARCHAR(4);
    DECLARE month VARCHAR(2);
...
if type = 'INSERT' THEN
    ...
    WHERE LEFT(MatriFourni, 10) = CONCAT(prefix, year, month);
    
    IF counter > 0 THEN
        -- Récupérer le dernier numéro utilisé dans la clé primaire
        SELECT MAX(RIGHT(MatriFourni, LENGTH(MatriFourni) - 10)) INTO counter
        FROM fournisseurs
        WHERE LEFT(MatriFourni, 10) = CONCAT(prefix, year, month);
        SET counter = counter + 1;
    END IF;
...
INSERT INTO fournisseurs (MatriFourni,nom,prenom,Email,Telephone,Adresse) VALUES (
primaryKey, nom, Prenom, email ,teelphone,adresse
);
ELSEIF type = 'UPDATE' THEN
UPDATE fournisseurs SET nom=nom,prenom=prenom,Email=email,Telephone=teelphone,Adresse=Adresse
WHERE MatriFourni=codefourni;

ELSEIF type = 'DELETE' THEN
DELETE FROM fournisseurs WHERE `MatriFourni` = codefourni;
END IF;
END //
DELIMITER //
...
DELIMITER //
CREATE PROCEDURE gestion_produit(IN type ENUM('INSERT','UPDATE','DELETE'),in codeproduit varchar(14),in designation text,in unite varchar(20))
BEGIN
DECLARE prefix VARCHAR(8); -- Modified prefix length to accommodate month
    DECLARE year VARCHAR(4);
    DECLARE month VARCHAR(2);
...
if type = 'INSERT' THEN
    ...
    WHERE LEFT(Matricule_produit, 10) = CONCAT(prefix, year, month);
    
    IF counter > 0 THEN
        -- Récupérer le dernier numéro utilisé dans la clé primaire
        SELECT MAX(RIGHT(Matricule_produit, LENGTH(Matricule_produit) - 10)) INTO counter
        FROM produits
        WHERE LEFT(Matricule_produit, 10) = CONCAT(prefix, year, month);
        SET counter = counter + 1;
    END IF;
...
INSERT INTO produits (Matricule_produit,designation,unite) VALUES (
primaryKey, designation, unite);
ELSEIF type = 'UPDATE' THEN
UPDATE produits SET designation=designation,unite=unite
WHERE Matricule_produit=codeproduit;

ELSEIF type = 'DELETE' THEN
DELETE FROM produits WHERE `Matricule_produit` = codeproduit;
END IF;
END //
DELIMITER //
