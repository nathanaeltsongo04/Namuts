...
DELIMITER//
	...
	BEGIN
		...
		IF U_OPERATION = 'INSERT' THEN
			SET CODE=(SELECT COALESCE(MAX(CODEUNITE),0)+1 FROM UNITE);
		...
		ELSEIF U_OPERATION = 'UPDATE' THEN
			UPDATE 
		ELSE
			SELECT 'Action Non Reconnu' as Message;
		END IF 
	END //
DELIMITER;
/*========================LES PROCEDURE STOCKEES GESTION_CATEGORIE===========================*/
DELIMITER//
CREATE PROCEDURE GESTION_CATEGORIE
(
	IN C_OPERATION TEXT,
	IN C_CODECATEGORIE INT,
	IN C_DESIGNATION TEXT,
	IN C_AUTEUR TEXT
)
BEGIN
	DECLARE CODE INT;
