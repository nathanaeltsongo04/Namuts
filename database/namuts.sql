...
BEGIN
						...
						IF        S_OPERATION= 'INSERT' THEN
							...
							ELSEIF    S_OPERATION= 'UPDATE' THEN
								...
								ELSEIF    S_OPERATION= 'DELETE' THEN
									...
									END IF;
								END;
 /*===================== PROCEDURE GESTION PRODUITS ==================================*/
 -- create TABLE gestion PRODUITS
					DELIMITER // CREATE PROCEDURE GESTION_PRODUIT ( IN P_OPERTATION TEXT, IN P_CODEPRODUIT VACHAR(14), IN P_DESIGNATION TEXT, IN P_UNITE INT, IN P_SEUIL INT, IN P_CATEGORIE INT, IN P_AUTEUR TEXT )
					BEGIN
						DECLARE
							PREFIX    VARCHAR(8); -- Modified PREFIX length to accommodate MONTH
							DECLARE   YEAR VARCHAR(4);
							DECLARE   MONTH VARCHAR(2);
							DECLARE   COUNTER INT;
							DECLARE   CODE VARCHAR(14); -- Adjusted to accommodate PREFIX, YEAR, MONTH, and COUNTER
 ...
 -- Vérifier si une clé primaire avec le préfixe, l'année, et le mois existe déjà
								SELECT    COUNT(*) INTO COUNTER FROM PRODUITS WHERE LEFT(CODEPRODUITS, 10) = CONCAT(PREFIX, YEAR, MONTH);
								IF        COUNTER > 0 THEN
 -- Récupérer le dernier numéro utilisé dans la clé primaire
									SELECT
										MAX(RIGHT(CODEPRODUITS,
										LENGTH(CODEPRODUITS) - 10)) INTO COUNTER
									FROM
										PRODUITS
									WHERE
										LEFT(CODEPRODUITS, 10) = CONCAT(PREFIX, YEAR, MONTH);
									SET       COUNTER = COUNTER + 1;
								END IF;
 -- Générer la clé primaire finale
								SET       CODE = CONCAT(PREFIX, YEAR, MONTH, LPAD(COUNTER, 4, '0'));
								INSERT    INTO PRODUITS (CODEPRODUITS, DESIGNATION, UNITE, SEUIL, CATEGORIE, AUTEUR) VALUES ( CODE, P_DESIGNATION, P_UNITE, P_SEUIL, P_CATEGORIE, P_AUTEUR);
								ELSEIF    P_OPERATION = 'UPDATE' THEN
									UPDATE PRODUITS SET DESIGNATION=DESIGNATION, UNITE=UNITE, SEUIL=SEUIL, CATEGORIE=CATEGORIE, AUTEUR=AUTEUR WHERE CODEPRODUITS=CODEPRODUIT;
